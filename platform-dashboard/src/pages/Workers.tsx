...
function formatWorkerZone(zone: string) {
  const rawZone = String(zone || '').trim()
  if (!rawZone) return 'Unknown Zone'
  ...
  const exportToCSV = () => {
    ...
    const rows = filteredWorkers.map((worker) => {
      ...
      return [
        ...
        worker.phone,
        zoneLabel,
        zoneLabel !== 'Unknown Zone' ? 'Live' : 'Offline',
      ]
    })
